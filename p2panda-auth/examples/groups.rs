...
use p2panda_auth::processor::{AuthExtension, AuthState, Store as GroupsStore, process};
...
type Resolver = StrongRemove<PublicKey, Hash, Operation<AppExtensions>, ()>;
...
impl Extension<AuthExtension> for AppExtensions {
    fn extract(header: &Header<Self>) -> Option<AuthExtension> {
        Some(header.extensions.auth.clone())
    }
}
...
async fn main() -> Result<()> {
    ...
    let groups_store = GroupsStore::<Operation<AppExtensions>>::default();
            ...
            while let Some(Ok(from_sync)) = sync_rx.next().await {
                match from_sync.event {
                    ...
                    SyncEvent::Operation(operation) => {
                        ...
                        let operation: Operation<AppExtensions> = *operation;

                        if let Err(err) = process::<_, _, Resolver>(&groups_store, &operation).await
                        {
                            println!();
                            println!("error: {err:?}");
                            println!();
                        ...
                        let groups_y = groups_store.get_state().await.unwrap();
                        print_group(groups_y, &operation).await;
                    }
                    _ => (),
                }
            }
    ...
    tokio::task::spawn(async move {
        while let Some(text) = line_rx.recv().await {
            let groups_y = groups_store.get_state().await.unwrap();
            let (group_id, action) = match text_2_action(&groups_y, public_key, text) {
                Ok(action) => action,
                Err(err) => {
                    println!();
                    println!("error: {err:?}");
            ...
            let extensions = AuthExtension { group_id, action };
...
fn text_2_action(
    y: &AuthState<Operation<AppExtensions>>,
    me: PublicKey,
    text: String,
...
async fn print_group(y: AuthState<Operation<AppExtensions>>, operation: &Operation<AppExtensions>) {
    let members = y
        .crdt
